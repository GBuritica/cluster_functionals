...
for(N in 1:500){
  for(j in 1:length(phi0)){
    ...
    for(i in 1:length(bu)){
      ...
      sumaalpha <- sapply( 1:floor(n/bu[i]), function(k) sum(   path[((k-1)*bu[i] + 1):(k*bu[i])]^alpha )  )
      ...
      th4     <- sortedsumaalpha[ max(1,floor( (n)/bu[i]^thr2[1]))+1 ] 
      ...
      if(p != alpha){
        ...
        th2     <- sortedsuma[ max(1,floor( (n)/bu[i]^thr2[1]))+1 ]                ## p-norm th.          ## a-norm th 
        ## index
        ...
        estim4       <-  1/mean( sapply(ind4, function(k)   g4(path[((k-1)*bu[i] +1):(k*bu[i])] , p)  )) 
      
        estim2       <-   estim3/estim2 
        ...
        estim1       <-  mean( sapply(ind4, function(k)     g1(path[((k-1)*bu[i] +1):(k*bu[i])] , p)  )) 
        ...
        if(j==1){
          infor1       <- rbind( infor1, c(estim1, 2, th2, bu[i], thr2[1], mode=j))
          infor1       <- rbind( infor1, c(estim2, 1, th3, bu[i], thr2[1], mode=j))
          if(p!=alpha) infor1       <- rbind( infor1, c(estim4, 3, th4, bu[i], thr2[1], mode=j))
        }
        else{
          infor2       <- rbind( infor2, c(estim1, 2, th2, bu[i], thr2[1], mode=j))
          infor2       <- rbind( infor2, c(estim2, 1, th3, bu[i], thr2[1], mode=j))
          if(p!=alpha) infor2       <- rbind( infor2, c(estim4, 3, th4, bu[i], thr2[1], mode=j))
        } 
      }
  }
}
...
g1 <- niceplot(infor1,estim=1/c1[1],title)
g2 <- niceplot(infor2,estim=1/c1[2],title)
#grid.arrange(g1,g2,nrow=1)

ggplot(infor2, aes( y = DATA,  type=as.factor(BL), fill = as.factor(TYPE) ))+
  geom_boxplot() + 
  geom_hline(yintercept =1/c1[2]) + ylim(0,3)
...
niceplot <- function(infor,estim, title){
  gg<- ggplot(infor, aes( y = 1/DATA,  type=as.factor(BL), fill = as.factor(TYPE) ))+ 
    ...
    geom_boxplot(#"#4271AE",
      ...
      scale_fill_manual(values = c("lightsteelblue3",'white',"gray"))
